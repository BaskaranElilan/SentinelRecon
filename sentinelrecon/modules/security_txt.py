...
from sentinelrecon.utils.util import clean_domain_input, clean_url, ensure_directory_exists, write_to_file, log_message
from sentinelrecon.config.settings import DEFAULT_TIMEOUT, RESULTS_DIR, EXPORT_SETTINGS
...
def run(target, threads, opts):
    ...
    console.print(Fore.GREEN + "      sentinelrecon - Security.txt Detection")
    console.print(Fore.GREEN + "=============================================\n")
    console.print(f"[*] Fetching {url}\n")

    try:
        r = requests.get(url, timeout=timeout, verify=False, headers={"User-Agent": "sentinelrecon/1.0"})
