...
from sentinelrecon.utils.util import clean_domain_input, resolve_to_ip, ensure_directory_exists, write_to_file
from sentinelrecon.config.settings import DEFAULT_TIMEOUT, API_KEYS, RESULTS_DIR, EXPORT_SETTINGS
...
def talos_lookup(dom, t):
    try:
        r = requests.get(f"https://talosintelligence.com/sb_api/query_lookup?query={dom}&query_type=domain", timeout=t, headers={"User-Agent": "sentinelrecon"}, verify=False)
        if r.status_code == 200:
