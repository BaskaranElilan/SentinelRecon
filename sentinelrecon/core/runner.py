...
from sentinelrecon.utils.report_generator import generate_report
from sentinelrecon.utils.util import clean_domain_input
...
def execute_script(script_name: str, target: str, threads: int = 1, module_opts: Dict | None = None, show_status: bool = True, quiet: bool = False) -> str:
    ...
    with ctx:
        ...
        cmd = [sys.executable, "-m", f"sentinelrecon.modules.{mod}", clean_domain_input(target), str(threads)]
        if module_opts:
