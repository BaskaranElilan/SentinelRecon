...
rm -f "$DEST_DIR/sentinelrecon" 2>/dev/null || true
# Also try common system paths in case of previous installs:
sudo rm -f /usr/local/bin/sentinelrecon 2>/dev/null || true
sudo rm -f /usr/bin/sentinelrecon 2>/dev/null || true
say "[STEP] (Optional) Remove PATH line in ~/.zshrc or ~/.bashrc if you added one."
say "[STEP] (Optional) Remove user-site Python deps via:"
say "  # if packaged: python3 -m pip uninstall sentinel-recon -y"
